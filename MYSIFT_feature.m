...
sigma0 = 1.6;
octave = 3;%6*sigma*k^(octave*s)<=min(m,n)/(2^(octave-2))
s = 3; % s = 3
D = cell(1,octave);
for i = 1:octave
    D(i) = mat2cell(zeros(row*2^(2-i)+2,colum*2^(2-i)+2,s),row*2^(2-i)+2,colum*2^(2-i)+2,s);
end

% first image in first octave is created by interpolating the original one.
temp_img=kron(img,ones(2)); %what is kron?
...
for i=1:octave
    temp_D=D{i};
    for j=1:s
        scale=sigma0*sqrt(2)^(1/s)^((i-1)*s+j); % k = sqrt(2)
        p=s*(i-1);
        figure(1);
        subplot(octave,s,p+j);
        f=fspecial('gaussian',[1,floor(6*scale)],scale);
        %gaussian filter, sigma = scale
        L1=temp_img;
        if(i==1&&j==1)
...
extrema(idx)=[]; %delete extrama below threshold
extr_volume=length(extrema)/4;
x=floor((extrema(3:4:end)-1)./(n./(2.^(extrema(1:4:end)-2))))+1;
y=mod((extrema(3:4:end)-1),m./(2.^(extrema(1:4:end)-2)))+1;
ry=y./2.^(octave-1-extrema(1:4:end));
...
%     scale=sigma0*sqrt(2)^(1/s)^((extrema(4*(i-1)+1)-1)*s+(extrema(4*(i-1)+2)));
